...
//	private VideoService videoService; /*injetamos a interface do service para buscar os metodos da classe service*/
//
//	/* metodo GET que busca um video por id */
//	@Override
//	public VideoDTO buscaVideoPorId(Long idVideo) { /*precisamos retornar um DTO, passando a id do video como parametro*/
//		log.info("[Iniciando] - Método buscaVideoPorId em VideoRestController");
//		Video video = videoService.buscaVideoPorIdService(idVideo); /*chamamos o metodo da service*/
//		log.info("[Finalizando] - Metodo buscaVideoPorId em VideoRestController");
//		return new VideoDTO(video); /*criamos um novo DTO com os dados do video temos como retorno do metodo acima*/
//	}
//
...
//		log.info("[Iniciando] - Método adicionaVideo em VideoRestController");
//		Video video = videoService.adicionaVideoService(videoForm.toEntity());/*chamamos o metodo da service, passando o form convertido em entidade*/
//		URI uri = uriBuilder.path("/").buildAndExpand(video.getId()).toUri(); /*criamos a URI, capturando o caminho, o parametro e no final convertemos tudo na uri final*/
//		log.info("[Finalizando] - Método adicionaVideo em VideoRestController");
//		return ResponseEntity.created(uri).body(new VideoDTO(video)); /*retornamos uma responseEntity, com status 201 e com um novo DTO para ser visualizado no body*/
//	}
//
...
//		log.info("[Iniciando] - Método editaVideo em VideoRestController");
//		if (!videoService.existsByIdService(idVideo)) { /*se o id do video não existir ou não for encontrado no banco de dados*/
//			return ResponseEntity.notFound().build(); /*irá retornar um 404 (não devolvendo nada no body, o build constroi um retorno vazio)*/
//		} /*se o id do video existir no banco de dados*/
//		video.setId(idVideo); /*vai buscar o video por aquele id e fazer a alteração de acordo com o que foi passado*/
//		video = videoService.adicionaVideoService(video); /*chama o metodo no service para para persistir a ação, no caso alterar o registro*/
//		log.info("[Finalizando] - Método editaVideo em VideoRestController");
//		return ResponseEntity.ok(video); /*retornando um status ok, confirmando a alteração*/
//	}
//
...
//			return ResponseEntity.notFound().build();/*irá retornar um 404 (não devolvendo nada no body, o build constroi um retorno vazio*/
//		} /*se o id do video existir no banco de dados*/
//		videoService.removeVideoService(idVideo); /*chama o metodo no service para para persistir a ação, no caso apagar o registro*/
//		log.info("[Finalizando] - Método deletaVideo em VideoRestController");
//		return ResponseEntity.noContent().build(); /*retornando um status No content (sem conteudo algum) e (não devolvendo nada no body, o build constroi um retorno vazio)*/
//	}
//
...
//		List<Video> videos = videoService.listaVideosService();/*precisamos buscar uma lista de registro na entiade, essa lista de videos vai buscar todos os registros persistidos no banco de dados*/
//		return VideoDTO.toList(videos);/*como queremos retonar uma lista de DTOs e não entidades, convertemos a mesma em DTO*/
//	}
//
//	@Override
//	public String working() { /*metodo simples so para testar o mapeamento no postman*/
//		return "working"; /*retornando uma string para confirmar resposta no body do postman*/
//	}
